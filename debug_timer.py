    ...
    while timer.is_active:
        ...
        if remaining_whole != last_whole:
            print(f"⏱️  Remaining: {remaining_whole:02d}s | "
                  f"Progress: {timer.get_progress():.1%} | "
...
def example_pause_resume() -> None:
    ...
    print(f"Remaining after pause: {timer.remaining.total_seconds():.1f}s "
          f"(should be same: {remaining_at_pause.total_seconds():.1f}s)")
...
def example_reset() -> None:
    ...
    print(f"Remaining after reset: {timer.remaining.total_seconds():.1f}s "
          f"(should be {duration.total_seconds():.1f}s)")
