...
class TestTimerTicking:
    ...
    def test_tick_clamps_at_zero(self, seconds: int):
        """Timer tick sets FINISHED when time consumed."""
        t = Timer(duration=timedelta(seconds=1))
        t.status = TimerStatus.RUNNING
        t._tick(seconds=seconds)  # Testing internal method
    ...
    def test_tick_ignored_when_inactive(self, status):
        """Timer tick has no effect outside RUNNING state."""
        t = Timer(duration=timedelta(seconds=3))
        t.status = status
        before = t.remaining
        t._tick(seconds=1)  # Testing internal method
    ...
    def test_thread_decrements_time(self, running_timer):
        ...
        time.sleep(0.7)  # Wait longer due to 0.5s resolution
    ...
    def test_timer_finishes_naturally(self):
        ...
        t = Timer(duration=timedelta(seconds=0.6))  # Adjusted for 0.5s resolution
        t.start()
        time.sleep(1.5)  # Wait long enough to finish with margin
        assert t.status == TimerStatus.FINISHED
